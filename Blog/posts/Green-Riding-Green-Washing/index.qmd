...
date: 2026-05-21
categories: [sport-ecology, sport-marketing]
description: "Preliminary analysis from my NASSM 2026 poster on sustainability messaging in mountain bike media."
page-layout: full
...
[Download Poster (PDF)](Pinkbike_Project_Poster.pdf)
...
This figure is included in the poster, but it shows the interaction between message framing (sustainability vs. not sustainability) and message source ("Review", "First Ride", "First Look", "Press Release")

``` {r}
#| label: fig-votes-sentiment
#| fig-cap: "Mean AFINN Sentiment Score by Article Type and Sustainability Mention"
#| warning: false

sentiment_visualization <- comments_sentiment %>%
  group_by(tags_series, article_has_sustainability_flag, is_reply) %>%
  summarise(
    mean_afinn = weighted_afinn(afinn_score, vote_weight),
    se         = sd(afinn_score, na.rm = TRUE) / sqrt(n()),
    n          = n(),
  ...
  mutate(
    comment_type   = if_else(is_reply, "Replies", "Top-level Comments"),
    sustainability = if_else(article_has_sustainability_flag,
                             "Mentions Sustainability", "No Sustainability Mentions")
  ) %>%
  ggplot(aes(x = tags_series, y = mean_afinn, fill = sustainability)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  geom_errorbar(
    aes(ymin = mean_afinn - se, ymax = mean_afinn + se),
    position = position_dodge(width = 0.7),
    width = 0.2
  ) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey40") +
  facet_wrap(~ comment_type) +
  scale_fill_manual(values = c("No Sustainability Mentions" = "#999999",
                               "Mentions Sustainability"    = "#006462")) +
  labs(
    title    = "Mean AFINN Sentiment Score by Article Type and Sustainability Mention",
    subtitle = "Vote-weighted means | Error bars show ±1 standard error | Pilot data (n = 19 articles)",
    x        = "Article Type",
    y        = "Mean AFINN Score",
    fill     = NULL,
    caption  = "Positive scores indicate more positive sentiment"
  ) +
  theme_minimal() +
  theme(
    ...
    strip.text      = element_text(face = "bold"),
    plot.title      = element_text(face = "bold", size = 14, colour = "#003B71")
  )
```
...
``` {r}
#| label: fig-votes-sentiment-2
#| fig-cap: "Unweighted vs. Vote-Weighted AFINN Sentiment"
#| warning: false

comments_sentiment %>%
  group_by(article_has_sustainability_flag,
           comment_has_sustainability_flag,
           is_reply) %>%
  summarise(
    unweighted = mean(afinn_score, na.rm = TRUE),
    weighted   = weighted_afinn(afinn_score, vote_weight),
    n          = n(),
    .groups    = "drop"
  ) %>%
  pivot_longer(cols = c(unweighted, weighted),
               names_to  = "scoring_method",
               values_to = "mean_afinn") %>%
  mutate(
    article_sustainability = if_else(
      article_has_sustainability_flag,
      "Sustainability article", "Non-sustainability article"
    ),
    comment_sustainability = if_else(
      comment_has_sustainability_flag,
      "Comment mentions sustainability", "Comment does not mention sustainability"
    ),
    comment_type = if_else(is_reply, "Reply", "Top-level")
  ) %>%
  ggplot(aes(x = comment_type, y = mean_afinn, fill = scoring_method)) +
  geom_col(position = position_dodge(width = 0.6), width = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey40") +
  facet_grid(article_sustainability ~ comment_sustainability) +
  scale_fill_manual(
    values = c("unweighted" = "#999999", "weighted" = "#006462"),
    labels = c("unweighted" = "Unweighted mean", "weighted" = "Vote-weighted mean")
  ) +
  labs(
    title    = "Unweighted vs. Vote-Weighted AFINN Sentiment",
    subtitle = "Weighted by net votes (upvotes - downvotes); minimum weight = 1",
    x        = NULL,
    y        = "Mean AFINN Score",
    fill     = NULL,
    caption  = "Pilot data (n = 19 articles)"
